...
注意：~~白名单保活~~ **已处理（2026-09-15）**：12 个垫片全部摘除，白名单清空。
...
| **⑤** | ~~**路径统一**：六模块的 CWD 相对路径全部改走 `config._project_root()` / `load_config()`~~ → **✅ 已完成（2026-09-15，`bc933c16`）**：34 处清零 + weekend 字符串路径 5 处 + weekend 配置接线 + weekend.md 3 处更正；棘轮基线已清空，新增违规立即红灯 | — |
...
| **⑩** | **健康 `store.py` 拆分**（H3）、~~周末 W1-W5 收尾~~ → **✅ W1/W2/W3 已随路径统一完成（`bc933c16`）**；W4 路由测试待补 | 中 | 体量工作，收益是后续可维护性 |
