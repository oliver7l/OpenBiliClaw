...
引用路径断链检查）。此前已修正 5 处过时（见上一条）；本次补完剩余批次核验：

- **agent / bilibili / cache / douban / extension / oss_research / youtube** 7 份文档：
  类名全部可在代码定位；引用的 TS/JS/资源路径从正确基准（`extension/src/`、`docs/specs/`）
  解析全部存在；`BILI_TASK_EXECUTE` 等扩展消息常量、`PlatformAdapter`、`CHROME_WEBSTORE_*`
  属扩展 TS / 环境变量 / 编译产物，非幽灵引用。
- 少量断链项均核实为「迁移/已删」的正确叙述（如 interview 旧路由、oss_research backfill
  迁移、health 导入脚本未入库），非文档错误。

**结论**：模块文档现状质量良好，除已修的 5 处外，其余全部一致、无幽灵类、无真实断链。
