...
- **结构化 LLM 生成健壮性封装**：新增 `llm/generation.py`，把散在各生成点的「调用→容错解析→手搓兜底」收敛成 `generate_structured / generate_json_list / generate_json_object`。默认 `reasoning_effort=""`（结构化任务关思考，直击 explore 之外 `delight`/discovery 策略在 512/2048 小预算下被推理 token 撑爆→空响应→静默产出 0 条的病根）；空内容或疑似截断（`completion_tokens`≈上限 / JSON 括号不平衡）时自动放大 `max_tokens`（×2，封顶 16384）+ 关思考重试；限流 backoff 后重试；`item_predicate` 契约校验，well-formed 但不合约定不静默放过；有界（默认 2 次）且**永不抛**——最终失败统一 WARN（`LLM <label> produced no valid output`），把隐形 0 变可见。复用现成 `json_utils` 与 `service` 错误分类，不新增依赖。`recommendation/delight.py` 批量打分率先接入（保留其已测的 `_extract_delight_entries`，经 `_parse_delight_entries` 适配器交给封装，空结果转 None 以触发重试）。新增 `test_llm_generation.py`（6 例：happy / 空恢复 / 截断恢复 / 限流重试 / 契约拒绝 / 垃圾不空转）。
