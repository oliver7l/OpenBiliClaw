...
> 不含实施方案——每个模块单独立方案后再动。
>
> **姊妹文档**：`docs/module-review-2026-09-15.md` 是「六模块逐个深挖」（日记/旅游/周末活动/阅读库/面试/健康监控），
> 本文是全局底账；批次编号两文共用（③④⑤… 见各自 §4/§8）。
...
**非缺陷**：`api/_interview_routes.py`（11 行）是期 3 有意留的兼容垫片（原在测试白名单）。**2026-09-15 更新**：12 个垫片已全部摘除，白名单清空。
...
`ruff` 已近清零；`mypy` 于 2026-09-15 收口（`1aeed497`：70 → 16，剩余全部为 saved_sync 死代码族待拍板）。09-13 那次审计**未复测 mypy**，故这项一直没人收口。
...
| ~~**③**~~ | ✅ **质量门禁**（2026-09-15 `1aeed497` 收口）：mypy 实测 70 → **16**（剩余＝saved_sync 原生保存死代码族，删/补待拍板）；ruff 3 条为既有 N806 | 低 | 纯机械修复，且是 AGENTS.md 要求的门禁 |
