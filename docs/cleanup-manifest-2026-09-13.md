...
> **状态**：✅ **A 组 + B1–B3 已执行**（2026-09-13，全部移入废纸篓，可还原）；B4（1.9G 回滚点）**未动**。
> 执行方式：`/usr/bin/trash`（macOS 14+ 系统命令，落点 `~/.Trash/`，已用探针文件验证机制可用；
> 注意沙箱**禁止读取** `~/.Trash`，所以无法用 `ls` 二次核对内容，只能依据工具自身输出）。
...
| # | 目标 | 大小 | 说明 | 判定 |
|---|------|------|------|------|
| B1 | `data/tax_frames/` | 2.8M | 全仓 grep（py/json/yaml/toml/md）**零命中**；另查全部 `data/*.db`（`grep --binary-files=text`）与 `data/*.json` 亦零命中 | ⚠️ 疑似孤儿 · **已移入废纸篓（09-13）** |
| B2 | `data/tax_frames2/` | 4.6M | 同上 | ⚠️ 疑似孤儿 · **已移入废纸篓（09-13）** |
| B3 | `data/tax_frames2_check/` | 2.3M | 同上（名字含 `_check`，疑似临时校验产物） | ⚠️ 疑似孤儿 · **已移入废纸篓（09-13）** |
