...
python -m openbiliclaw.runtime.douyin_producer --login           # 首次扫码登录
python -m openbiliclaw.runtime.douyin_producer --once --limit 20  # 抓一轮推荐流
python -m openbiliclaw.runtime.douyin_producer --jingxuan --once   # 抓精选页（免登录）
python -m openbiliclaw.runtime.douyin_producer --likes --once      # 抓喜欢的视频（需登录）
python -m openbiliclaw.runtime.douyin_producer --favorites --once  # 抓收藏的视频（需登录）
python -m openbiliclaw.runtime.douyin_producer --cdp-port 9222    # 连真实 Chrome
```
