...
- **v0.3.192 数据层拆分（总库+子库）**：推荐流 4 表（`content_cache` / `recommendations` / `user_feedback` / `xhs_observed_urls`）从主库 `data/openbiliclaw.db` 迁出至独立子库 `data/pool.db`。采集器（`_obc_connect`）与 `Database`（所有连接自动 `ATTACH pool.db`）读写推荐流均落到子库，与主库（events / diary / saved 等）零锁交集；迁移脚本 `scripts/migrate_pool_db.py` 负责备份、复制与主库 DROP。`discovery_candidates` 仍在主库。
