...
- **对外接口**：CLI `openbiliclaw interview` 命令组 + API `/api/interview/*` + Web「面试」tab，CLI 与 API 复用同一 `InterviewEngine`
...
| 求职面试备战 | **InterviewEngine**（`interview/`） | 三层求职知识库（CSV + SQLite）只读检索 + 速记卡/日志/建档；CLI + `/api/interview/*` + Web tab |
