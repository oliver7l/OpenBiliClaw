...
### 2.6 🎯 求职面试备战模块 (Interview Prep)

**目标**：把用户自建的三层求职知识库（`求职知识库/`）接入 OpenBiliClaw，成为可查询、可复盘、可扩展的面试备战系统模块。

- **三层知识库**：01_原始资料库（底层只读，工作资料/解码文本/简历/书籍）→ 02_方向知识库（中层方法论：广告算法/推荐/数据科学/SQL/面试方法论）→ 03_岗位弹药库（上层按岗位：JD 拆解/面试攻略/预测题库/速成包）
- **系统引擎**：`_系统_知识库引擎` 7 张 CSV 数据表（岗位/项目/真实数字/日志/题索引/全库索引/概念关系）+ `knowledge.db` + 检索脚本 + 规范（岗位匹配评估/录入规范/新增岗位流程）
- **模块能力**：岗位总览 / 全文检索（三层 + 工作资料 + 规范）/ 真实数字（口径权威源）/ 项目库 / 面试题索引 / 方向文档 / 全库索引（重建）/ 面试日志（追加）/ 面试速记卡（含岗位定制弹药）/ 健康检查 / 新岗位建档
- **数据原则**：原始材料只读原位保留、日志只追加、建档幂等、索引覆盖重建；数字答案永远以真实数字表为权威
- **对外接口**：CLI `openbiliclaw interview` 命令组 + API `/api/interview/{job,study,review}/*`（旧前缀双挂载兼容）+ Web「面试」tab，CLI 与 API 复用同一 `InterviewEngine`
...
| 求职面试备战 | **InterviewEngine**（`interview/`） | 三层求职知识库（CSV + SQLite）只读检索 + 速记卡/日志/建档；CLI + `/api/interview/job/*` + Web tab |
