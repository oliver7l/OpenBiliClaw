#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从 面试资料总库.db 移除 category='幻灯片笔记' 的文档（已复制到独立 幻灯片原始库.db）。

顺序（同 kb_remove_*_from_master.py，关键坑已踩过）：
...
def main():
    ...
    if db == SRC and not args.no_backup:
        ...
        bak = os.path.join(ARCHIVE, f"面试资料总库_移除幻灯片笔记前_{time.strftime('%Y%m%d_%H%M%S')}.db")
    ...
    before = {
        ...
        "幻灯片笔记": c.execute(f"SELECT count(*) FROM doc WHERE category IN ({CAT_PH})", CATS).fetchone()[0],
    }
    ...
    c.commit()
    print(f"  [OK] 移除 幻灯片笔记 doc={len(doc_ids)} / chunk={len(chunk_ids)}")
    ...
    print("\n=== 结果（前 → 后）===")
    for k in before:
        print(f"  {k:8s}: {before[k]} → {after[k]}  (减 {before[k]-after[k]})  {'OK' if (k!='幻灯片笔记' and before[k]-after[k]==(len(doc_ids) if k=='doc' else len(chunk_ids))) or (k=='幻灯片笔记' and after[k]==0) else ''}")
