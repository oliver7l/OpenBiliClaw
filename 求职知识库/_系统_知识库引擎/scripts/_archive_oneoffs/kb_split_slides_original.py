...
# -*- coding: utf-8 -*-
"""
把 面试资料总库.db 里 category='幻灯片笔记' 的文档（文档级原始抽取），连同其
doc_chunk / doc_content / doc_vector(向量) / doc_fts(全文索引) 完整复制进独立的 幻灯片原始库.db。

设计原则（同 kb_split_books.py / kb_split_direction.py）
- 只增不删：源库（面试资料总库.db）只读 ATTACH，绝不修改；新库为独立新建文件。
- 保留原始 id：doc / doc_chunk / doc_vector 的 id 全部原样复制，FK 关联不重排。
- 自包含：新库重建 doc_fts（外部内容 FTS）与 doc_vector（vec0），可独立做全文+语义检索。
- 不复制全局运维表。

注：此库是幻灯片笔记的「文档级原始源」。幻灯片级 OCR 在 幻灯片笔记.db（活源），
主程序收口在 data/interview.db.slide（与 幻灯片笔记.db 逐行一致）。三层职责分离。

用法
  python kb_split_slides_original.py 检查
  python kb_split_slides_original.py 执行 [--force]
"""
import os
import sys
import sqlite3
...
TGT = os.path.join(ROOT, "求职知识库/_系统_知识库引擎/数据/幻灯片原始库.db")
SLIDE_CATS = ("幻灯片笔记",)
...
def cat_ids(src):
    ph = ",".join("?" * len(SLIDE_CATS))
    return [r[0] for r in src.execute(
        f"SELECT id FROM doc WHERE category IN ({ph}) ORDER BY id", SLIDE_CATS)]
...
def cmd_check(src):
    ...
    print("=== 检查：将复制到 幻灯片原始库.db 的数据 ===")
    print(f"  doc(幻灯片笔记)  : {doc_n}")
...
def cmd_run(force):
    ...
    print(f"\n[完成] 幻灯片原始库已生成: {TGT}  ({os.path.getsize(TGT)/1024/1024:.1f} MB)")
