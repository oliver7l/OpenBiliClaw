...
# -*- coding: utf-8 -*-
"""
把 面试资料总库.db 里 category ∈ ('书籍','技术书籍') 的文档，连同其
doc_chunk / doc_content / doc_vector(向量) / doc_fts(全文索引) 完整复制进独立的 书籍库.db。
...
- 自包含：新库重建 doc_fts（外部内容 FTS，rebuild 自 doc）与 doc_vector（vec0 自动建影子表），
  所以书籍库可独立做全文检索 + 语义向量检索。
- 不复制全局运维表（ingest_log / processed_notes / unprocessed）。

用法
  python kb_split_books.py 检查            # 统计将复制哪些数据（dry-run）
  python kb_split_books.py 执行 [--force]  # 真正生成 书籍库.db
"""
import os
import sys
import sqlite3
...
TGT = os.path.join(ROOT, "求职知识库/_系统_知识库引擎/数据/书籍库.db")
BOOK_CATS = ("书籍", "技术书籍")
# 需整体复制并保留 id 的表（内容链路）
CONTENT_TABLES = ("doc", "doc_chunk", "doc_content")
# 虚拟表（schema 来自源库，数据单独迁移）
...
def cmd_check(src):
    ...
    print("=== 检查：将复制到 书籍库.db 的数据 ===")
    print(f"  doc(书籍)        : {doc_n}")
...
def cmd_run(force):
    ...
    ids = book_ids(src)
    cids = book_chunk_ids(src, ids)
    assert ids, "未找到书籍文档，终止"
    ph = ",".join("?" * len(ids))
    cph = ",".join("?" * len(cids))

    # 1) 建 schema（从源库取精确 CREATE 语句，保证列一致）
    ...
    # 3) 复制向量（chunk_id + 序列化 float32 blob 原样搬）
    rows = tgt.execute(
    ...
    print(f"\n[完成] 书籍库已生成: {TGT}  ({os.path.getsize(TGT)/1024/1024:.1f} MB)")
