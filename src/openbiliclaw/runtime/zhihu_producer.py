"""Runtime Zhihu discovery producer and recommendation feed scheduler.
...
The recommendation-feed half calls ``zhihu feed --json`` on a 24-hour cycle
and inserts new answers / pins into ``content_cache`` so they become available
in the recommendation pool. Run as a script (``python -m
openbiliclaw.runtime.zhihu_producer``) to start the feed loop.
"""
...
def _run_once() -> dict[str, Any]:
    return run_once_for_platform(
        ...
        insert_rows=lambda conn, rows: insert_rows_slim(
            conn, rows, metric="like_count", with_body_text=True
        ),
    )
