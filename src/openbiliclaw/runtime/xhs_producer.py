"""Soul-driven xhs search task producer + recommendation feed scheduler.

Two responsibilities live in this module:

1. **Task producer** (``XhsTaskProducer``): runs on the same loop as the
   continuous refresh controller. Once per throttle window (default 1h) it
   reads the current SoulProfile, asks an LLM to rewrite interest tags into
   xhs-flavored keywords, and enqueues one ``search`` task per keyword into
   ``XhsTaskQueue``. The extension's background dispatcher polls the queue,
   opens each search page in a hidden tab, and reports results back —
   closing the Soul → Discovery loop for xiaohongshu.

2. **Feed scheduler** (``run_forever`` / ``_run_once``): calls
   ``xhs feed --json`` every 24 hours and inserts new notes into
   ``content_cache`` with fresh ``xsec_token``, so they become available in
   the recommendation pool immediately.
"""
...
def _run_once() -> dict[str, Any]:
    return run_once_for_platform(
        ...
        insert_rows=lambda conn, rows: insert_rows_slim(conn, rows, metric="like_count"),
    )
