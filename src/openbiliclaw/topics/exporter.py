...
_STOP_WORDS: set[str] = {
    ...
    "什么", "怎么", "为什么", "可以", "因为", "所以", "但是", "如果", "虽然",
    "然后", "就是", "还是", "或者", "以及", "并且", "而且", "不过", "只是",
    "已经", "正在", "将要", "可能", "应该", "需要", "知道", "觉得", "认为",
    "其实", "真的", "确实", "当然", "显然", "毕竟", "总之", "另外", "此外",
    "比如", "例如", "关于", "对于", "通过", "根据", "按照", "由于", "基于",
    ...
    "she", "her", "what", "which", "who", "whom", "when", "where",
...
def fuse_topic_content(
    ...
    for platform, platform_items in by_platform.items():
        combined_text = " ".join(
            f"{item.title} {item.summary} {item.content_text[:2000]}"
...
def _build_fusion_draft(
    ...
    lines.append("")
    lines.append(f"> 自动生成于跨平台内容融合分析，覆盖 {len(by_platform)} 个平台，"
