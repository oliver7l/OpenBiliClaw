...
def linuxdo_discovery_items_to_contents(
    ...
    *,
    ...
    for item in items:
        ...
        published = _scalar_value(item, "published_at", "created_at")
        views, likes, replies, engagement_available = _engagement(item)
        candidate = DiscoveredContent(
            ...
            discovered_at=str(published).strip() if published is not None else "",
        )
        ...
        if not existing.discovered_at and candidate.discovered_at:
