...
用法：
    # 查看今日待读
    python -m openbiliclaw.interview.study.cli today

    # 查看题库统计
    python -m openbiliclaw.interview.study.cli stats

    # 查看待看队列
    python -m openbiliclaw.interview.study.cli queue

    # 标记题目已读
    python -m openbiliclaw.interview.study.cli read <question_id> [--mastery understood] [--notes "笔记"]

    # 标记题目掌握
    python -m openbiliclaw.interview.study.cli master <question_id>

    # 标记需要复习
    python -m openbiliclaw.interview.study.cli review <question_id>

    # 查看题目详情
    python -m openbiliclaw.interview.study.cli show <question_id>

    # 创建阅读计划
    python -m openbiliclaw.interview.study.cli plan --name "秋招冲刺" --daily 5

    # 查看阅读进度
    python -m openbiliclaw.interview.study.cli progress
"""
...
from openbiliclaw.interview.study.models import MasteryLevel  # noqa: E402
from openbiliclaw.interview.study.store import InterviewQuestionStore  # noqa: E402
...
def cmd_today(args: argparse.Namespace) -> None:
    ...
    for i, q in enumerate(questions, 1):
        ...
        if q.tags:
            print(f"     标签：{q.tags}")
    print(f"\n共 {len(questions)} 题")
    print("\n标记已读：python -m openbiliclaw.interview.study.cli read <id>")
    print("标记掌握：python -m openbiliclaw.interview.study.cli master <id>")
