...
def build_llm_registry(
    ...
    *,
    ...
    registry.fallback_enabled = bool(getattr(config, "fallback_enabled", False))
    registry.fallback_provider = str(getattr(config, "fallback_provider", "")).strip().lower()
...
def build_embedding_service(
    ...
    try:
        ...
        from obc_llm.embedding import EmbeddingCache, EmbeddingService, SupportsEmbed
...
def _build_dedicated_embedding_provider(
    ...
    if candidate == "openrouter":
        ...
        chat_openrouter = config.openrouter
        return (
            OpenRouterProvider(
                api_key=api_key,
                model=effective_model,
...
def summarize_registry(config: LLMConfig, registry: LLMRegistry) -> RegistrySummary:
...
def _maybe_openai_provider(config: LLMConfig, overrides: dict[str, LLMProvider]) -> LLMProvider | None:
    if "openai" in overrides:
        return overrides["openai"]
    auth_mode = config.openai.auth_mode.strip().lower()
    if auth_mode == "codex_oauth":
        from obc_llm.codex_auth import get_valid_codex_token, load_codex_credentials
        ...
        return OpenAIProvider(
            ...
            model=config.openai.model or "gpt-4o",
...
def _maybe_claude_provider(config: LLMConfig, overrides: dict[str, LLMProvider]) -> LLMProvider | None:
    if "claude" in overrides:
        return overrides["claude"]
    if not config.claude.api_key.strip():
        return None
    return ClaudeProvider(
        api_key=config.claude.api_key,
        model=config.claude.model or "claude-sonnet-4-20250514",
        timeout=float(config.timeout),
    )
...
def _maybe_deepseek_provider(
    ...
    return DeepSeekProvider(
        ...
        model=config.deepseek.model or "deepseek-v4-flash",
...
def _maybe_gemini_provider(config: LLMConfig, overrides: dict[str, LLMProvider]) -> LLMProvider | None:
    if "gemini" in overrides:
        return overrides["gemini"]
    api_key = config.gemini.api_key.strip() or _gemini_env_api_key()
    if not api_key:
        return None
...
def _maybe_ollama_provider(config: LLMConfig, overrides: dict[str, LLMProvider]) -> LLMProvider | None:
    if "ollama" in overrides:
        return overrides["ollama"]

    raw_base_url = config.ollama.base_url.strip()
    model = config.ollama.model.strip()
    ...
    return OllamaProvider(
        api_key=config.ollama.api_key or "ollama",
        model=model or "llama3",
        base_url=base_url,
        timeout=float(config.timeout),
        num_ctx=int(config.ollama.num_ctx),
    )


def _ollama_is_chat_capable(config: LLMConfig) -> bool:
    ...
    """
    if config.ollama.model.strip():
        return True
    if config.default_provider.strip().lower() == "ollama":
        return True
    if config.fallback_provider.strip().lower() == "ollama":
        return True
    for module in ("soul", "discovery", "recommendation", "evaluation"):
        module_cfg = getattr(config, module, None)
...
def _maybe_openrouter_provider(
    ...
    return OpenRouterProvider(
        ...
        model=config.openrouter.model or "openai/gpt-4o-mini",
