...
from obc_discovery._protocols import CoverFetchError, ImageCache
...
# Module-level image cache — set by the main project during initialization.
_image_cache: ImageCache | None = None


def set_image_cache(cache: ImageCache | None) -> None:
    """Set the global cover-image fetch/cache provider.

    Called by the main project during runtime initialization.
    """
    global _image_cache
    _image_cache = cache
...
async def prepare_cover_image_input(
    *,
    ...
    cache = _image_cache
    if cache is None:
        logger.info("Skipping cover image for %s: no image cache configured", cid)
        return None

    try:
        async with asyncio.timeout(max(1, int(timeout_seconds))):
            data, _content_type = await cache.get_or_fetch_cover_bytes(url)
