#!/usr/bin/env python3
"""将求职知识库岗位弹药库题库 Markdown 解析入库（interview.db.interview_questions）。

【重要】本脚本与 interview/study/seed_iq_questions.py 互为镜像，勿混用：

  ✅ 本脚本
     - 数据源：求职知识库/03_岗位弹药库/ 下各岗位备战目录的题库/速成包/预测/攻略 Markdown
     - 目标库：interview.db（interview_questions 表，199 条）
     - 用途：把外部题库 md 解析为结构化题，存入库表，供 A 速记卡 / B /study/kb-questions 使用

  ❌ seed_iq_questions.py（勿混淆）
...
用法
----
    python3 scripts/import_kb_questions_to_interview_db.py          # dry-run 预览
    python3 scripts/import_kb_questions_to_interview_db.py --apply   # 实际写入
    python3 scripts/import_kb_questions_to_interview_db.py --apply --force  # 清空重导
"""
...
# 2026-09-17：万声、比亚迪已升为项目根级工作区（编号 03_/04_），基准改为项目根
# （原路径 万声科技-面试准备/03_速成包/…、比亚迪-面试准备/03_速成包/… 已随目录重组失效）
QA_FILES_ROOT = [
    "03_万声科技/05_面试临场/万声音乐推荐算法_预测题库.md",
    "04_比亚迪/05_面试临场/比亚迪营销岗_预测题库.md",
]
...
def main() -> int:
    ...
    try:
        ...
        total = 0
        files_done = 0
        sources = [(ARMORY, r) for r in QA_FILES] + [(PROJECT_ROOT, r) for r in QA_FILES_ROOT]
