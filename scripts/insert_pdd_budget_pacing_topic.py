#!/usr/bin/env python3
"""将「发券预算控制与节奏 Pacing」专题写入 interview.db 的 kb_documents 表（doc_type='面试专题'）。

直连 SQLite，不依赖 InterviewEngine（避免触发 LLM 429）。
撤销：DELETE FROM kb_documents WHERE doc_type='面试专题' AND topic LIKE '发券预算控制与节奏%';
"""
...
SRC = ROOT / "求职知识库" / "03_岗位弹药库" / "拼多多-面试准备" / "02_面试备战资料" / "发券预算控制与节奏Pacing_专题.md"

TOPIC = "发券预算控制与节奏Pacing：工业界做法完整专题"
CATEGORY = "拼多多"
CHAPTER = "因果推断·Uplift·智能补贴·预算控制"
TAGS = "预算控制,Pacing,发券,因果推断,推荐系统,拼多多,智能补贴,节流,强化学习,BCORLE,AIM"
DOC_TYPE = "面试专题"
