...
**TraeBar（本项目自己的托盘，2026-09-19 建）**：既然给现成 App 加托盘不可行，
就绕开它——`tray/TraeBar.app` 是原生 Swift 菜单栏应用（无 Dock 图标，LSUIElement）：
菜单 = 当前登录 + 账号切换子菜单（点击即切，当前账号打勾置灰）+ 全部签到/余额刷新 +
打开助手 + 退出。底层全部复用 `scripts/twa_*`（业务零重复），签到/切换完成后发系统通知。
给助手注入托盘的路已探明不可行：v0.1.3 前端资源嵌在 Rust 二进制里且未开
`withGlobalTauri`，JS 拦不了关窗事件，二进制补丁不现实。想开机自启：
