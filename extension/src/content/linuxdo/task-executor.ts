...
export interface LinuxdoTaskItem {
  ...
  author_url?: string;
  summary?: string;
  /** Opt-in full post body (first post raw/cooked), plumbed to 阅读库. */
  body_text?: string;
  category?: string;
  tags: string[];
  views?: number;
...
export interface LinuxdoExecuteMessage {
  ...
  /** Opt-in: return the seed topic's full first-post body as ``body_text``. */
...
/** Full first-post body from a topic-detail envelope (opt-in capture_body). */
function captureTopicBody(raw: unknown): string {
  const root = asRecord(raw);
  const postStream = asRecord(root.post_stream);
  const firstPost = recordArray(postStream.posts)[0] ?? {};
  return stripHtml(
...
async function runDiscovery(message: LinuxdoExecuteMessage): Promise<{
  ...
  for (const seed of seeds) {
    ...
    try {
      ...
      if (message.capture_body === true) {
        const seedItem = normalizeLinuxdoTopic(detail, {
          scope,
          strategy,
          sourceInput: seed,
        });
        if (seedItem) {
          const body = captureTopicBody(detail);
          seedItem.body_text = body || seedItem.summary || "";
          rows.push(seedItem);
        }
      }
